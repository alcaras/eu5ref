...
Every gated tool (Tech Planner, values-planner, value-path, the CountryLens
on list pages) evaluates gates against ONE facts model, `Facts`, built as
**1337 setup ⊕ imported save ⊕ your overrides**:

- **`one` kinds** (cul / rel / cap / gov / subj / parl / ctype / age) hold one
  value and are always known once a country is picked. **`set` kinds** (mcg,
  reform, estate, iomem, axis, dlc, law, priv, policy, var, adv, unl, `?`)
  are atom → bool maps where an absent atom is *unknown* → the gate stays
  conditional. Only a definite false hides anything.
- `build_planner.py` emits `kinds` — the vocabulary the "⚙ Your country"
  panel is built from: per kind, the values some gate actually tests, with
  counts and bundles (`f`: a culture is cul + cgrp + lang, an area is every
  capital tier). The panel is generic — a new predicate kind in triggers.py
  needs only a KINDS entry to get UI. Also emitted: `modkeys` (label per
  modifier key) and per-node `m` = `[key, value, polarity]` triples.
- **Scripted triggers are inlined** (`_scripted()` in triggers.py, 874
  inlinable bodies; parameterised `$X$` bodies stay in `_SCRIPTED_SKIP`),
  and every predicate that still falls through becomes a stable `['?', id,
  label]` (15 today, e.g. `num_locations > 0`) which the panel shows under
  "Other conditions" with an "assume ✓" action — never silently true.
- **URL codec** (`overridesToParams`): one param per overridden kind
  (`cul=basque`, `cap=assam_area`, `reform=partitio_reform~-anatolian_beylik`,
  `q=` for `?`), `~` between set atoms, leading `-` = asserted false;
  `show=sure|all` (default = yours + conditional). Links carry facts, never
  the save.
- **Save import** (`src/lib/save-import.ts` + the header rail in
  `Base.astro`, `src/lib/save-store.ts`): a *melted* `.eu5` (pdx.tools →
  Melt; the game's own saves are compressed) is streamed through a
  line-oriented scanner in the browser — top-level blocks skipped with one
  `indexOf`, database entries sliced whole and regex-read — ~0.7 s for
  350 MB, nothing uploaded. `resolve()` turns it into a snapshot: `Facts` for
  every Real country (culture/religion bundles via planner.json `kinds`,
  capital tiers via `public/geo.json` from `build_geo.py`, subject type from
  `diplomacy_manager` dependencies, IOs from `all_members`, active estates
  from `estate_manager`) plus a full **seat** (laws, privileges, reforms,
  values, institutions, researched advances) for each played country.
  Set kinds the save knows completely (reform/estate/iomem; law/priv/adv for
  seats) assert ✗ for every tested value not present; DLC matches on label
  or stays unknown; unified cultures (`mcg`) are not read. Stored in
  `localStorage['eu5ref-save']` (~1.3 MB — long lists only for seats, or it
  will not fit) with `eu5ref-save-pref` `{enabled, seat}`; `activeSeat()` /
  `saveFacts(tag)` / `seatAsStart()` are what pages read, and every page
  shows a `save: X · off` chip. Defaults it fills in when the URL is silent:
  planner country / researched / age tab; value tools' STARTS record for
  the seat; CountryLens country. Tested headlessly (esbuild-bundle the
  scanner, run in node) against a 1.1.1 MP save — the only local fixture;
  shapes for 1.3.x subject relations are handled but unverified.
...
- `public/planner.json` (Tech Planner graph), `public/geo.json` (location →
  area → region → sub-continent → continent, for the save importer) and
  `public/search.json` (header search) are generated by build_planner.py /
  build_geo.py / build_entities.py and fetched lazily client-side — they are
  NOT dataset envelopes and are excluded from entity aggregation by living
  in public/, not src/data/.
- **Melted-save shapes** (don't re-discover): one tab per depth; top-level
  blocks close on a bare `}` line; `metadata.flag="TAG={` names the saving
  player and `played_country={ name= country=<id> }` (repeated near EOF)
  every human; `countries.tags` maps id → TAG and IO `all_members` ids are
  those same ids verbatim (no bit-masking); `compatibility.locations` is a
  one-line list whose index+1 is the location id; a block's last line has
  no trailing newline unless you keep it (`block()` does — a scalar on the
  last line was silently missed before).
