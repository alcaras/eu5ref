...
- `<EntityTable entities columns pagePrefix>` — schema-driven table; column
  kinds: `name | text | num | chips | facet | mods | rich | pairs`. Emits
  `data-search` + `data-f-<facet>` on rows.
...
- `<Term id label?>` — one linked entity reference (plain span if no page).
- `<Mods>`, `<Rich>` — see above.
- `<Base title active eyebrow pageMark pageStats description>` — shell.
