...
def prepend_changelog(version: str, ts: str, lines: list[str]) -> None:
    ...
    if CHANGELOG.exists():
        ...
        existing = "# Changelog\n\nGenerated by `scripts/changelog.py`. Each entry compares the current data with the snapshot taken for the previous patch.\n\n"
    ...
    for f in files:
        ...
        if f in SUMMARY_ONLY:
            ...
            file_lines = diff_list(prev, cur)
              ...
              and isinstance(prev.get("entities"), list) and isinstance(cur.get("entities"), list)):
            # eu5ref dataset envelope: diff the entity list per-entry
            file_lines = diff_list(prev["entities"], cur["entities"])
        elif isinstance(prev, dict) and isinstance(cur, dict):
